...
/**
 * The score shows if and how a project uses static analysis with MyPy. The score is based on the
 * following features.
 *
 * <ul>
...
public class MyPyScore extends FeatureBasedScore {
   ...
   *
   * @see <a href="http://www.mypy-lang.org/">MyPy overview</a>
   */
  ...
  public ScoreValue calculate(Value<?>... values) {
    Value<Boolean> usesMyPyChecks =
        findValue(
            values,
            USES_MYPY_SCAN_CHECKS,
            "Hey! You have to tell me if the project uses MyPy checks!");
    Value<Boolean> runsMyPyScans =
        findValue(
            values, RUNS_MYPY_SCANS, "Hey! You have to tell me if the project runs MyPy scans!");
    Value<Languages> languages =
        findValue(values, LANGUAGES, "Hey! You have to tell me which languages the project uses!");

    ScoreValue scoreValue = scoreValue(MIN, usesMyPyChecks, runsMyPyScans, languages);
