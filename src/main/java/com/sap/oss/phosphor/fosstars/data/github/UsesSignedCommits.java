...
/**
 * This data provider checks if a project uses signed commits. The check would be based on a density
 * ratio compared against a percentage threshold ({@link #SIGNED_COMMIT_PERCENTAGE_THRESHOLD}).
 */
public class UsesSignedCommits extends CachedSingleFeatureGitHubDataProvider<Boolean> {
  ...
  /** The threshold in % of number of signed commits against total number of commits in a year. */
   ...
   * density ratio. The density ratio is the percentage calculation of number of signed commits
   * against the total number of commits in the year. If the density ratio in % crosses a certain
   * percentage threshold ({@link #SIGNED_COMMIT_PERCENTAGE_THRESHOLD}), then it says that the
   * project uses signed commits. Otherwise, it says that the project does not use signed commits.
   *
  ...
  private boolean askGithub(GitHubProject project) throws IOException {
    ...
    List<Commit> signedCommits =
        yearCommits.stream().filter(Commit::isSigned).collect(Collectors.toList());
